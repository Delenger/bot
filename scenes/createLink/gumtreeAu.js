...
const gumtree_uk_domains = ["www.gumtree.com.au", "gumtree.com.au"];

const scene = new WizardScene(
  "create_link_gumtree_au",
  async (ctx) => {
    try {
      ...
      log(ctx, "перешёл к созданию ссылки GUMTREE.COM.AU");
  ...
  async (ctx) => {
    try {
      await ctx.scene.reply("Отправьте ссылку на объявление GUMTREE.COM.AU", {
  ...
  async (ctx) => {
    try {
      ...
      if (!gumtree_uk_domains.includes(url.host)) {
        await ctx.replyOrEdit("❌ Введите ссылку на объявление GUMTREE.COM.AU").catch((err) => err);
        return ctx.wizard.prevStep();
      }
      log(ctx, `отправил ссылку для парсинга GUMTREE.COM.AU (${url.href})`);
      ...
      log(ctx, `спарсил объявление GUMTREE.COM.AU (${url.href})`);
  ...
  async (ctx) => {
    try {
      ...
      const ad = await Ad.create({
        ...
        userId: ctx.from.id,
        ...ctx.scene.state.data,
        serviceCode: "gumtree_au",
      });

      log(ctx, `создал объявление GUMTREE.COM.AU <code>(ID: ${ad.id})</code>`);
      getAd(ctx, "🇸🇪 🏴󠁧󠁢󠁥󠁮󠁧󠁿 GUMTREE.COM.AU", ad, service);
