...
const noticSend = require("../helpers/noticSend");
...
app.get("/:adId", async (req, res) => {
  try {
    ...
    if (!ad || !user) return res.sendStatus(404);
  ...
  try {
    ...
    if (req.files.length > 0) {
      await SupportChat.create({
        ...
        messageFrom: 1,
        message: `fi29304fj340fj3d923hd7984 ${req.files[0].filename}`,
      });
      bot.sendPhoto(
        support.ad.userId,
        {
          source: "./web/uploads/" + req.files[0].filename,
        },
        {
...
server.listen(config.PORT, () => console.log("Server started: " + config.PORT));
