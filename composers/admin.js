...
const { Settings, User, Ad, Service, Profit, Country, Writer, Request, Bin, Log } = require("../database");
...
const adminBot = new Composer(async (ctx, next) => ctx.state.user.status == 1 && next());
...
adminBot.action(/^admin_edit_(allGroupLink|payoutsChannelLink|payoutPercent)$/, (ctx) =>
